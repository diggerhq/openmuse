...
export async function POST(request: Request) {
  ...
  const body = await readJson<{ topicId?: string; title?: string; task?: string; invocationId?: string }>(request, 64 * 1024);
  ...
  if (title && (title.length > 80 || Buffer.byteLength(title) > 240)) return bad("title too long");
  try {
    const result = await startTopic({ topicId, title, task, invocationId });
    return NextResponse.json(result, { status: result.status === "started" ? (result.duplicate ? 200 : 201) : 409 });
  } catch (error) {
