...
import { type Document, type DocumentMeta, memory } from "@/lib/memory";
...
export interface Recall {
  ...
  readonly overview?: {
    readonly text: string;
    readonly sources: Array<{ id: string; title: string; revision: string; updatedAt: string }>;
  };
}
...
export async function recallForCoordinator(): Promise<Recall> {
  ...
  return {
    ...
    overview: {
      ...
      sources: topics
        .slice(0, 50)
        .map((t) => ({ id: t.id, title: t.title, revision: t.revision, updatedAt: t.updatedAt })),
    },
  };
}
