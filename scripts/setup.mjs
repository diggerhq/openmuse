...
// secret and the agent installation secret into .env.local, links or creates
// the OpenComputer project, uploads the agent secret, deploys both agents to
// Development, and prints how to deploy the app. It never prints the
// OpenComputer key.
//
...
try {
  ...
  if (!origin) throw new Error("Pass --origin https://<the app's public https origin> (a tunnel for local runs, the Vercel URL for a deployment).");
  const url = new URL(origin);
  if (url.protocol !== "https:" || url.pathname !== "/") throw new Error("--origin must be an https origin without a path");
  env.OPENMUSE_APP_ORIGIN = url.origin;

  const generated = [];
  for (const [name, bytes] of [["OPENMUSE_OWNER_SECRET", 24], ["OPENMUSE_COOKIE_SECRET", 32]]) {
    if (!env[name] || rotate) { env[name] = secret(bytes); generated.push(name); }
  }
  if (!env.OPENMUSE_AGENT_SECRET) { env.OPENMUSE_AGENT_SECRET = secret(32); generated.push("OPENMUSE_AGENT_SECRET"); }
  if (!env.OPENMUSE_INSTALLATION_ID) { env.OPENMUSE_INSTALLATION_ID = randomBytes(8).toString("hex"); generated.push("OPENMUSE_INSTALLATION_ID"); }

  const config = JSON.parse(await readFile(process.env.OPENCOMPUTER_CONFIG ?? join(homedir(), ".opencomputer/config.json"), "utf8"));
  ...
  await cli(["secrets", "set", "OPENMUSE_AGENT_SECRET", "--environment", "development",
    "--allow-origin", env.OPENMUSE_APP_ORIGIN, "--value-stdin"], { input: env.OPENMUSE_AGENT_SECRET });
...
Deploy to Vercel: set these environment variables from .env.local in the Vercel project, then \`vercel deploy --prod\`:
  OPENMUSE_OWNER_SECRET, OPENMUSE_COOKIE_SECRET, OPENMUSE_AGENT_SECRET, OPENMUSE_INSTALLATION_ID, OPENMUSE_APP_ORIGIN,
  OPENCOMPUTER_API_URL, OPENCOMPUTER_PROJECT_ID, OPENCOMPUTER_ENVIRONMENT, OPENMUSE_COORDINATOR_AGENT, OPENMUSE_WORKER_AGENT,
  and OPENCOMPUTER_API_KEY (an OpenComputer API key; locally the app reads ~/.opencomputer/config.json instead).
