...
import { SESSION_COOKIE, constantTimeEquals, verifySession, type OwnerSession } from "@/lib/auth/cookie";
...
export function ownerSession(request: Request): OwnerSession | null {
...
export type Guard = { ok: true; session: OwnerSession } | { ok: false; response: NextResponse };

export function requireOwner(request: Request, options: { mutation?: boolean } = {}): Guard {
  const session = ownerSession(request);
  if (!session) return { ok: false, response: NextResponse.json({ error: "unauthorized" }, { status: 401 }) };
  if (options.mutation) {
    if (!sameOrigin(request)) return { ok: false, response: NextResponse.json({ error: "bad_origin" }, { status: 403 }) };
