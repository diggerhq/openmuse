// OpenComputer management API, held server-side with the API key. Only the
// routes the public api-edge exposes today are used.
...
async function requestWithStatus<T>(path: string, init: RequestInit & { idempotencyKey?: string } = {}): Promise<{ status: number; body: T }> {
  ...
  if (!response.ok) {
    ...
    const code = typeof error === "object" && error?.code ? error.code : typeof error === "string" ? error : `http_${response.status}`;
    const message = typeof error === "object" && error?.message ? error.message : `OpenComputer ${path} failed (${response.status})`;
...
export const oc = {
  async createSession(agent: string, idempotencyKey: string) {
    const result = await requestWithStatus<{ session: { id: string; executionMode?: "microvm" | "workerd"; status: string }; deployment?: { id: string } }>(
      "/sessions",
      { method: "POST", body: JSON.stringify({ agentId: `${agent}@${env().environment}`, source: "api" }), idempotencyKey },
    );
  ...
  async events(id: string, after: number, signal?: AbortSignal) {
    return (await request<{ events: OcEvent[] }>(`/sessions/${encodeURIComponent(id)}/events?after=${after}`, { signal })).events;
  },
  ...
  project() {
    return request<{ project: { id: string; environments: Array<{ name: string; agentId: string; activeDeploymentId?: string }> } }>(
