...
export type CreateResult = { readonly status: "created"; readonly document: Document } | { readonly status: "exists" | "deleted" };

export interface MemoryStore {
  list(resource: Resource): Promise<DocumentMeta[]>;
  get(resource: Resource, id: string): Promise<Document | null>;
  create(resource: Resource, id: string, body: { title: string; text: string; summary?: string }): Promise<CreateResult>;
  /** Replace text (and optionally summary) if `expectedRevision` is current. */
  replace(resource: Resource, id: string, body: { text: string; summary?: string }, expectedRevision: string, writer: Writer): Promise<SaveResult>;
  /** Owner-only: title and write policy. */
  patch(resource: Resource, id: string, patch: { title?: string; agentWrites?: "enabled" | "disabled" }, expectedRevision: string): Promise<SaveResult>;
}
