...
import { LIMITS, documentIdValid, type CreateResult, type Document, type DocumentMeta, type MemoryStore, type Resource, type SaveResult, type Writer } from "@/lib/memory/adapter";
...
async function call(url: string, init: RequestInit = {}): Promise<{ status: number; etag: string | null; body: unknown }> {
  const response = await fetch(url, {
    ...init,
    headers: { "x-api-key": env().apiKey, accept: "application/json", ...(init.body ? { "content-type": "application/json" } : {}), ...(init.headers ?? {}) },
    redirect: "error",
...
export const platformStore: MemoryStore = {
  async list(resource) {
    ...
    do {
      const result = await call(`${base(resource)}?${environment()}${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`);
  ...
  async create(resource, id, body): Promise<CreateResult> {
    const result = await call(`${base(resource)}/${encodeURIComponent(id)}?${environment()}`, {
      method: "PUT", headers: { "if-none-match": "*" }, body: JSON.stringify({ title: body.title, text: body.text, summary: body.summary ?? "" }),
    });
  ...
  async replace(resource, id, body, expectedRevision, _writer: Writer): Promise<SaveResult> {
    ...
    const result = await call(`${base(resource)}/${encodeURIComponent(id)}?${environment()}`, {
      method: "PUT", headers: { "if-match": quoted(expectedRevision) }, body: JSON.stringify(body.summary === undefined ? { text: body.text } : body),
    });
    ...
    if (result.status === 412) {
      ...
      return current ? { status: "conflict", text: current.text, summary: current.summary, revision: current.revision } : { status: "rejected", reason: "not_found" };
    }
    if (result.status === 404) return { status: "rejected", reason: "not_found" };
    if (result.status === 413) return { status: "rejected", reason: "too_large", bytes: Buffer.byteLength(body.text), maxBytes: LIMITS[resource] };
    throw new Error(`memory replace failed (${result.status})`);
  },
  async patch(resource, id, patch, expectedRevision): Promise<SaveResult> {
    const result = await call(`${base(resource)}/${encodeURIComponent(id)}?${environment()}`, {
      method: "PATCH", headers: { "if-match": quoted(expectedRevision) }, body: JSON.stringify(patch),
    });
    if (result.status === 200) {
      const document = asDocument(resource, result.body);
      return { status: "saved", revision: document.revision, bytes: document.bytes };
