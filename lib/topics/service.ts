...
import { allEvents, oc, type OcEvent, type OcSession } from "@/lib/oc/client";
import { activeDeploymentId, activeTurn, createOrReuseSession, queueTurn, readSession, sessionUsable } from "@/lib/oc/sessions";
import { readState, updateState, type TopicRecord } from "@/lib/state/store";
import { memory, type Document, type SaveResult } from "@/lib/memory";
...
export type StartTopicResult =
  | { readonly status: "started"; readonly topicId: string; readonly title: string; readonly sessionId: string; readonly turnId: string; readonly duplicate: boolean; readonly newTopic: boolean }
  | { readonly status: "refused"; readonly reason: "archived" | "not_found"; readonly topicId: string; readonly topics?: string[] };

function slug(title: string): string {
  return title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 40) || "topic";
}

function topicIdFor(title: string, invocationId: string): string {
  return `${slug(title)}-${createHash("sha256").update(invocationId).digest("hex").slice(0, 6)}`;
}
...
async function ensureTopicRecord(topicId: string, title: string): Promise<TopicRecord> {
  return updateState((state) => {
    ...
    const record: TopicRecord = { id: topicId, title, createdAt: new Date().toISOString(), archived: false, previousWorkerSessionIds: [], invocations: {}, recalledRevision: {} };
...
async function ensureWorkerSession(topic: TopicRecord): Promise<string> {
  ...
  await updateState((state) => {
    ...
    const previous = current.workerSessionId ? [...current.previousWorkerSessionIds, current.workerSessionId] : current.previousWorkerSessionIds;
    return {
      state: { ...state, topics: { ...state.topics, [topic.id]: { ...current, workerSessionId: created.id, workerDeploymentId: deploymentId, previousWorkerSessionIds: previous } } },
      result: undefined,
    };
  });
  return created.id;
...
async function queueWorkerTurn(topicId: string, sessionId: string, text: string, idempotencyKey: string) {
  ...
  if (recall.topic) {
    ...
    await updateState((state) => {
      ...
      return { state: { ...state, topics: { ...state.topics, [topicId]: { ...current, recalledRevision: { ...current.recalledRevision, [sessionId]: revision } } } }, result: undefined };
    });
  }
  return queueTurn(sessionId, composeTurnInput(recall, text), idempotencyKey);
}

export async function startTopic(input: { topicId?: string; title?: string; task: string; invocationId: string }): Promise<StartTopicResult> {
  const state = await readState();
  let topicId = input.topicId;
  let title = input.title ?? "";
  let newTopic = false;
  if (!topicId) {
    topicId = topicIdFor(title, input.invocationId);
  ...
  await updateState((current) => {
    ...
    return { state: { ...current, topics: { ...current.topics, [topicId]: { ...topic, invocations: { ...topic.invocations, [input.invocationId]: turn.turnId } } } }, result: undefined };
  });
  return { status: "started", topicId, title, sessionId, turnId: turn.turnId, duplicate: turn.duplicate, newTopic };
}

// Owner follow-up from the topic panel: another turn on the same session.
export async function continueTopic(topicId: string, text: string) {
  const document = await memory().get("topics", topicId);
  ...
  const sessionId = await ensureWorkerSession(record);
  return { sessionId, ...(await queueWorkerTurn(topicId, sessionId, text, randomUUID())) };
}
...
export async function stopTopic(topicId: string) {
  ...
  return oc.turn(record.workerSessionId, "[stop] The owner pressed Stop. Stop the current work; reply in one short sentence with where things stand and save notes if you have verified anything.", randomUUID(), "interrupt");
}
...
export interface TopicSummary {
  ...
  readonly work?: { readonly status: string; readonly turns: number; readonly activeTurnId?: string; readonly lastTurnStatus?: string };
}
...
export async function listTopics(): Promise<TopicSummary[]> {
  ...
  return Promise.all(documents.map(async (document) => {
    ...
    return {
      id: document.id, title: document.title, summary: document.summary, updatedAt: document.updatedAt,
      archived: record?.archived ?? false, agentWrites: document.agentWrites, workerSessionId: record?.workerSessionId,
...
function workStatus(session: OcSession) {
  ...
  return { status: session.status, turns: session.turns.length, activeTurnId: active?.id, lastTurnStatus: last?.status };
}
...
export async function topicDetail(topicId: string): Promise<TopicDetail | null> {
  ...
  const record = (await readState()).topics[topicId];
  const session = record?.workerSessionId ? await readSession(record.workerSessionId) : null;
  const events: OcEvent[] = session ? await allEvents(session.id) : [];
  return {
    topic: {
      id: document.id, title: document.title, summary: document.summary, updatedAt: document.updatedAt,
      archived: record?.archived ?? false, agentWrites: document.agentWrites, workerSessionId: record?.workerSessionId,
...
export async function agentSaveNotes(sessionId: string, body: { text: string; summary?: string }): Promise<SaveResult | { status: "rejected"; reason: "not_bound" }> {
  const state = await readState();
  ...
  const revision = result.status === "saved" ? result.revision : result.status === "conflict" ? result.revision : undefined;
  if (revision) {
    ...
    await updateState((current) => {
      ...
      return { state: { ...current, topics: { ...current.topics, [topic.id]: { ...record, recalledRevision: { ...record.recalledRevision, [sessionId]: revision } } } }, result: undefined };
    });
  }
  return result;
}

export async function agentSaveProfile(sessionId: string, text: string): Promise<SaveResult | { status: "rejected"; reason: "not_bound" }> {
  const state = await readState();
  if (state.coordinator?.sessionId !== sessionId || !state.coordinatorRecalledRevision) return { status: "rejected", reason: "not_bound" };
  const result = await memory().replace("profile", "owner", { text }, state.coordinatorRecalledRevision, { kind: "agent", sessionId });
  const revision = result.status === "saved" ? result.revision : result.status === "conflict" ? result.revision : undefined;
  if (revision) await updateState((current) => ({ state: { ...current, coordinatorRecalledRevision: revision }, result: undefined }));
  return result;
}

export async function ownerEditNotes(topicId: string, body: { text: string; summary?: string }, expectedRevision: string): Promise<SaveResult> {
...
export async function setArchived(topicId: string, archived: boolean, expectedRevision: string): Promise<SaveResult> {
  const result = await memory().patch("topics", topicId, { agentWrites: archived ? "disabled" : "enabled" }, expectedRevision);
  if (result.status !== "saved") return result;
  const record = (await readState()).topics[topicId];
  if (archived && record?.workerSessionId) {
    try { await oc.end(record.workerSessionId); } catch { /* already ended */ }
  }
  await updateState((state) => {
    const current = state.topics[topicId];
    if (!current) return { state, result: undefined };
    const next: TopicRecord = archived
      ...
      : { ...current, archived: false, workerSessionId: undefined, previousWorkerSessionIds: current.workerSessionId ? [...current.previousWorkerSessionIds, current.workerSessionId] : current.previousWorkerSessionIds };
...
export async function replaceWorker(topicId: string): Promise<{ endedSessionId?: string }> {
  ...
  await updateState((state) => {
    const current = state.topics[topicId];
    if (!current) return { state, result: undefined };
    return { state: { ...state, topics: { ...state.topics, [topicId]: { ...current, workerSessionId: undefined, previousWorkerSessionIds: [...current.previousWorkerSessionIds, ended] } } }, result: undefined };
  });
